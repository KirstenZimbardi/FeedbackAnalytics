...
#load data
demog = read.csv("demog.csv", header=TRUE, stringsAsFactors=FALSE)
AcP.df = read.csv("AcPdf.csv", header=TRUE, stringsAsFactors=FALSE)
...
```{r, echo=FALSE}
temp = length(unique(AcP.df$SubmissionID))
```
...
temp = dcast(AcP.df, course + sem ~ report)
#temp
df = subset(demog, Course.Grade !="withrawn")
