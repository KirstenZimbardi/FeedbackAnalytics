...
```{r}
df = mach.learn
# removing cases w'out a R3 mark
df = subset(df, y == 1 | y == 0)

# pulling out students with complete records for report marks
## removed - put back in and adapt if mach learning dosn't cope with NaN in featres
#studs = unique(df$StudentID)
#stud.reports = as.data.frame.matrix(with(df, addmargins(table(StudentID, report),2)))
#stud.complete = row.names(subset(stud.reports, Sum == 4))

# splitting data
stud.split = splitter(df$StudentID, 60)
df.train = df[which(df$StudentID %in% stud.split[[1]]),]
df.cv = df[which(df$StudentID %in% stud.split[[2]]),]
df.test = df[which(df$StudentID %in% stud.split[[3]]),]
